...
def transfercargo(args, name, anount, player, ship):
    ...
    x = ttyio.inputinteger(
        f"{{var:promptcolor}}{name}: {{var:inputcolor}}", ship.manifest[name], **kwargs
    )
    if x > ship.manifest[name]:
        emoji = res["emoji"] if "emoji" in res else ""
        ttyio.echo(
            "The manifest does not contain {x} {bbsengine.util.pluralize(x, res['singular'], res['plural'], emoji)}",
            level="error",
        )
        return False
...
def main(args, **kwargs):
    ...
    if logs > player.timber or timber < 0:
        ttyio.echo(
            "You only have {bbsengine.util.pluralize(player.timber, 'board', 'boards'}!"
        )
    else:
