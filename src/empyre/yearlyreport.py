...
def main(args, **kwargs):
    ...
    p4 = (
        ((random.random() * 175) + player.soldiers)
        * player.foundries
        // (2 - player.combatvictorycount)
    )
    # p5=((rnd(1)*200)+la/30)*f%(5)/(2-ff)
    p5 = (
        ((random.random() * 200) + player.land // 30)
        * player.shipyards
        // (2 - player.combatvictorycount)
    )
    ...
    noblegifts = (
        int(player.taxrate * (random.random() * player.nobles)) * 100 // 4
    )  # xx
    if noblegifts < 1 and player.nobles > 67:
        a = player.nobles // 5
        player.nobles -= a
        io.echo(
            "{blue}%s{/blue}" % (util.pluralize(a, "noble defects", "nobles defect"))
        )
    ...
    soldierpay = int(
        (player.soldiers * (player.combatvictorycount + 2))
        + (player.taxrate * player.palaces * 10) // 40
    )  # py
    ...
    if givenpeople > grain:
        ...
        io.echo(
            "You only have {} to give to your people.".format(
                util.pluralize(grain, **grnres)
            )
        )
    ...
    immigration = int(
        random.random() * (player.serfs // 15) + 10
    )  # z(5)=int(rnd(1)*(x(19)/15)+10)
    ...
    io.echo()
    io.echo(
        f"{{labelcolor}} Soldier's Pay:  {{valuecolor}}{soldierpay:>6n}"
    )  # % ("{:>6n}".format(soldierpay)))
    io.echo(
        f"{{labelcolor}} Palace Rent:    {{valuecolor}}{palacerent:>6n}"
    )  # %s" % ("{:>6n}".format(palacerent)))
    io.echo(
        f"{{labelcolor}} Noble's Gifts:  {{valuecolor}}{noblegifts:>6n}"
    )  # % ("{:>6n}".format(noblegifts)))
    io.echo()

    io.echo(f"{{labelcolor}}INCOME --- {{valuecolor}}{receivables:>6n}{{normalcolor}}")
    io.echo()
    io.echo(
        f"{{labelcolor}} Markets:        {{valuecolor}}{p2:>6n}"
    )  # % ("{:>6n}".format(p2))) # p2 markets
    io.echo(
        f"{{labelcolor}} Mills:          {{valuecolor}}{p3:>6n}"
    )  # % ("{:>6n}".format(p3))) # p3 mills
    io.echo(
        f"{{labelcolor}} Foundries:      {{valuecolor}}{p4:>6n}"
    )  # % ("{:>6n}".format(p4))) # p4 foundries
    io.echo(
        f"{{labelcolor}} Shipyards:      {{valuecolor}}{p5:>6n}"
    )  # % ("{:>6n}".format(p5))) # p5 shipyards
    io.echo(
        f"{{labelcolor}} Taxes:          {{valuecolor}}{taxes:>6n}"
    )  # % ("{:>6n}".format(taxes))) # tg/taxes
    io.echo()

    if receivables == payables:
        io.echo(
            f"{{lightgreen}}Break Even: {payables:>6n}"
        )  # %s{/all}" % ("{:>6n}".format(payables)))
    elif receivables > payables:
        io.echo(
            f"{{lightgreen}}Profit:     {receivables - payables:>6n}"
        )  # %s{/all}" % ("{:>6n}".format(receivables-payables))) # (pluralize(receivables-payables, "credit", "credits")))
    elif receivables < payables:
        io.echo(
            f"{{lightred}}Loss:       {payables - receivables:>6n}"
