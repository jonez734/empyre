...
def main(args, **kwargs):
    ...
    if player.dragons > 0:
        if (
            io.inputboolean(
                "{var:promptcolor}Unleash a dragon? {var:optioncolor}[yN]{var:promptcolor}: {var:inputcolor}",
                "N",
                **kwargs,
            )
            is False
    ...
    if len(damages) == 0:
        io.echo(
            f"{{var:labelcolor}}Your dragon *did not* damage the empyre of {{var:valuecolor}}{otherplayer.moniker}{{var:normalcolor}}"
        )
        return True
