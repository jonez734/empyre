...
from bbsengine6.listbox import Listbox, ListboxItem, ListboxResult, init as ListboxInit
...
class Ship(object):
    ...
    def save(self, commit=True, moniker=None):
        ...
        database.update(
            self.args, "empyre.__ship", pk, ship, primarykey="moniker", updatepk=True
        )
        if commit is True:
...
def _verifyshipname(args, moniker, **kwargs) -> bool:
    ...
    def _work(conn):
        with database.cursor(conn) as cur:
            ...
            cur.execute(sql, dat)
            io.echo(
                f"verifyShipNameFound.100: {database.mogrifysql(cur, sql, dat)=}",
                level="debug",
            )
            if cur.rowcount == 0:
                return False
...
def selectship(args, **kwargs):
    ...
    class EmpyreShipListboxItem(ListboxItem):
        def __init__(self, rec: dict, width: int):
            super().__init__()
            pool = getattr(EmpyreShipListboxItem, "pool", None)
            self.ship = Ship(args, pool=pool)
