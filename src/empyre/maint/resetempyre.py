from bbsengine6 import io, database, util as bbsutil, member
...
def removeplayer(args, player):
    ...
    if cur.rowcount == 0:
        ...
        io.echo(f"empyre has {bbsutil.pluralize(cur.rowcount, 'player', 'players')}")
...
def removeship(args, player, **kwargs):
    ...
    io.echo(
        f"{{normalcolor}}empyre has {{valuecolor}}{bbsutil.pluralize(rowcount, **shipres)}"
    )
    if rowcount == 0:
        return True
    if (
...
def removeisland(args, player):
    ...
    if io.inputboolean("remove islands? [yN]: {inputcolor}", "N") is True:
        ...
        cur.execute(sql)
        dbh.commit()
        io.echo(f"{bbsutil.pluralize(cur.rowcount, 'island', 'islands')} deleted.")
