...
def _merccount(cursor, **kwargs):
    sql: str = "select count(*) as cnt from empyre.mercs"
    cursor.execute(sql)
    rec = cursor.fetchone()
    return rec["cnt"] if rec is not None else 0


def generate_merc_teams_with_costs(
    min_members=10,
    max_teams=10,
    max_members_per_team=50,
    min_cost_per_member=10,
    max_cost_per_member=50,
):
    """
...
def main(args: object, player=None, **kwargs):
    ...
    for team, details in merc_teams.items():
        print(
            f"{team}: {details['members']} members, {details['cost_per_member']} coins per member, total cost {details['total_cost']} coins."
        )
