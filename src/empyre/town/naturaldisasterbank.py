...
def main(args, player, **kwargs):
    util.heading(
        "natural disaster bank"
    )  # , titlecolor="{bggray}{white}", hrcolor="{darkgreen}")
    lib.setbottombar(
        args, "natural disaster bank", player=player
    )  # "{bggray}{white}%s{/all}" % ("bank".ljust(terminalwidth)))
    ...
    if amount > cr:
        io.echo(
            f"{{var:labelcolor}}Get REAL! You only have {{var:valuecolor}}{util.pluralize(cr, **coinres)}{{var:labelcolor}}!"
        )
        return
    ...
    io.echo(
        f"{{var:normalcolor}}You now have {{var:valuecolor}}{util.pluralize(player.coins, **coinres)}{{var:normalcolor}} and {{var:valuecolor}}{util.pluralize(cr, 'credit', 'credits', emoji=':moneybag:')}{{var:normalcolor}}"
    )
