...
def main(args, player, **kwargs):
    ...
    if player.turncount > 2:
        if player.soldierpromotioncount > 1:
            io.echo(
                "Nice try, but we keep our promotion records up to date, and they show that your eligible soldiers have already been promoted! Just wait until King George hears about this!"
            )
            player.soldiers -= 20
    ...
    if player.soldiers < 10:
        io.echo(
            "None of your soldiers are eligible for promotion to Noble right now.{F6}"
        )
        return
    ...
    io.echo(
        "{F6}{var:normalcolor}Good day, I take it that you are here to see if any of your soldiers are eligible for promotion to the status of noble.{F6}"
    )
    io.echo(
        "{f6:3}Well, after checking all of them, I have found that %s eligible.{f6}"
        % (util.pluralize(promotable, "soldier is", "soldiers are"))
    )
    if promotable == 0:
        return

    ch = io.inputboolean(
        "{var:promptcolor}Do you wish them promoted? {var:optioncolor}[yN]{var:promptcolor}: {var:inputcolor}",
        "N",
        **kwargs,
    )
    io.echo("{/all}")
