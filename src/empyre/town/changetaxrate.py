...
def main(args, player, **kwargs):
    ...
    x = io.inputinteger("{var:promptcolor}tax rate: {var:inputcolor}", player.taxrate, **kwargs)
    io.echo("{/all}")
    if x is None or x < 1:
        io.echo("no change")
        return True
    if x > 50:
        io.echo(
            "King George looks at you sternly for trying to set such an exhorbitant tax rate, and vetoes the change.",
            level="error",
        )
        return True
