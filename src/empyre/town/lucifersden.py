...
def main(args, **kwargs):
    ...
    if player.coins > 10000 or player.land > 15000:
        io.echo(
            "{F6}I checked our inventory and we have plenty of souls. Maybe we can deal some other time.{f6:2}"
        )
        return True
    ...
    io.echo(
        "{f6}this module does not currently save player data after use", level="info"
    )
    ...
    while not done:
        ...
        if player.serfs < 1000:
            io.echo(
                "{normalcolor}You must have at least {valuecolor}%s{normalcolor} to gamble here!"
                % util.pluralize(1000, "serf", "serfs")
            )
            done = True
            break
        io.echo(
            "{F6}{normalcolor}You have :moneybag: {valuecolor}%s{/all} and {valuecolor}%s{normalcolor}"
            % (
                util.pluralize(player.coins, "coin", "coins"),
                util.pluralize(player.serfs, "serf", "serfs"),
            )
        )
        bet = io.inputinteger(
            "{promptcolor}Bet how many coins? (No Limit): {inputcolor} ", **kwargs
        )
        io.echo("{normalcolor}")
