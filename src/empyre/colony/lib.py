...
def insert(args, colony):
    return database.insert(
        args, "empyre.__colony", colony, returnid=True, primarykey="name"
    )
...
def edit(args, colony):
    ...
    io.echo(
        f"{{var:optioncolor}}[N]{{var:labelcolor}}ame: {{var:valuecolor}}{colony['name']}",
        end="",
    )
    if colony["name"] != _colony["name"]:
        io.echo(
            f" {{var:labelcolor}}(was {{var:valuecolor}}{_colony['name']}{{var:labelcolor}})"
        )
    else:
        io.echo()
    io.echo(
        f"{{var:optioncolor}}[I]{{var:labelcolor}}sland ame: {{var:valuecolor}}{colony['islandname']}",
        end="",
    )
    if colony["islandname"] != _colony["islandname"]:
        io.echo(
            f" {{var:labelcolor}}(was {{var:valuecolor}}{_colony['islandname']}{{var:labelcolor}})"
        )
    else:
        io.echo()
