...
def main(args, **kwargs):
    ...
    quests = {
        ...
        "questofgods": {"title": "The Quest of the Gods", "callback": None},
        "evilcult": {"title": "Eradicate the Evil Cult", "callback": None},
        "islandofspice": {"title": "Search for the Island of Spice", "callback": None},
        "birdcity": {"title": "Quest for the Legendary Bird City", "callback": None},
        "mountainsideship": {
    ...
    for q in quests.values():
        ...
        if (
            ...
            and module.checkmodule(args, callback, buildargs=False, **kwargs) is True
        ) is True:
            io.echo("true")
    ...
    while not done:
        ...
        ch = io.inputchar(
            f"{{var:promptcolor}}quest {{var:optioncolor}}[{options}]{{var:promptcolor}}: {{var:inputcolor}}",
            options,
            "Q",
            help=help,
            **kwargs,
        ...
        if "intro" in quest:
            util.filedisplay(
                args, data.get(quest["intro"])
            )  # bbsengine.buildfilepath(lib.QUESTDIR, quest["intro"]))  # data.get(quest["intro"]))

        if module.runsubmodule(args, quest["callback"], **kwargs) is True:
            if "win" in quest:
                util.filedisplay(
                    args, util.buildfilepath(libempyre.QUESTDIR, quest["win"])
                )
            else:
            ...
            if "fail" in quest:
                util.filedisplay(
                    args, util.buildfilepath(libempyre.QUESTDIR, quest["fail"])
                )
            else:
