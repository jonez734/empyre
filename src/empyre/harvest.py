...
def main(args, **kwargs):
    ...
    x = int(
        (
            player.land * player.weatherconditions
            + (random.random() * player.serfs)
            + player.grain * player.weatherconditions
        )
        / 3
    )
    ...
    io.echo(
        f"{{f6}}{{normalcolor}}This year's harvest is {{valuecolor}}{util.pluralize(x, **grainres)}{{f6}}"
    )
    ...
    io.echo(
        f"{{normalcolor}}Your people require {{valuecolor}}{util.pluralize(serfsrequire, **grainres)}{{normalcolor}} of grain this year{{/all}}{{f6}}"
    )
    ...
    if serfsgiven < 1:
        io.echo(
            f"{{(normalcolor}}Giving {{valuecolor}}{util.pluralize(howmany, **grainres)}{{normalcolor}} of grain)"
        )
        serfsgiven = 0
    ...
    io.echo(
        f"{{normalcolor}}Your army requires {util.pluralize(player.armyrequires, **grainres)}{{normalcolor}} this year and you have {{valuecolor}}{util.pluralize(player.grain, **grainres)}"
    )
    price = int(6 // player.weathercondition)
    price = int(price / (player.land / 875) + 1)
    grainres["price"] = price
    ###    libempyre.trade(args, player, "grain", **grainres) # "bushel", price, "bushel", "bushels", emoji=":crop:")
    ...
    player.armygiven = io.inputinteger(
        "{promptcolor}Give them how many?: {/all}{inputcolor}",
        player.armyrequires,
        **kwargs,
    )
    if player.armygiven > player.grain:
        io.echo(
            f"You do not have enough grain! Your army has been given {util.pluralize(player.grain, **grainres)}"
        )
    ...
    io.echo(f"You gave your army {util.pluralize(player.armygiven, **grainres)}{{f6}}")

    horsesres = player.getresource(
        "horses", singular="horse requires", plural="horses require"
    )
    if player.horses > 0:
